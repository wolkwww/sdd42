...
Status uart_set_divisors(LPC_UART_TypeDef *UARTx, uint32_t baudrate)
{
	...
	for (mulFracDiv = 1; mulFracDiv <= 15; mulFracDiv++)
	{
		for (dividerAddFracDiv = 0; dividerAddFracDiv <= 15; dividerAddFracDiv++)
		{
			...
			if ((temp % baudrate) > (baudrate / 2)) diviser++;

			if (diviser > 2 && diviser < 65536)
			{
				calcBaudrate = temp / diviser;

				if (calcBaudrate <= baudrate) relativeError = baudrate - calcBaudrate;
				else relativeError = calcBaudrate - baudrate;
...
void UART_GenIntHandler(LPC_UART_TypeDef *UARTx)
{
	...
	if (intsrc)
	{
		...
		UARTx->ACR |= ((intsrc & UART_IIR_ABEO_INT) ? UART_ACR_ABEOINT_CLR : 0) | ((intsrc & UART_IIR_ABTO_INT)
		    ? UART_ACR_ABTOINT_CLR : 0);
